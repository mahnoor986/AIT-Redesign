...
export const getProgramsByCategory = (category: string) => programs.filter(p => p.category === category);
export const getAllCategories = () => Array.from(new Set(programs.map(p => p.category)));
